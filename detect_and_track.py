...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, save_with_object_id=False, path=None,offset=(0, 0),kpi2_text=""):
    for i, box in enumerate(bbox):
        ...
        if save_with_object_id:
            ...
            with open(path + '.txt', 'a') as f:
                f.write(txt_str)
            kpi2_text.write(txt_str)
    return img
#..............................................................................


def detect(weights=ROOT / 'best.pt',  # model path or triton URL
        source=ROOT ,  # file/dir/URL/glob/screen/0(webcam)
        kpi1_text="",
        kpi2_text="",
        stframe=None,
        save_img=False):
    view_img, save_txt, imgsz, trace, colored_trk, save_bbox_dim, save_with_object_id= opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace, opt.colored_trk, opt.save_bbox_dim, opt.save_with_object_id
    save_img = not opt.nosave and not source.endswith('.txt')  # save inference images
    webcam = source.isnumeric() or source.endswith('.txt') or source.lower().startswith(
        ('rtsp://', 'rtmp://', 'http://', 'https://'))
    df=pd.DataFrame(columns=['idcategoria','idobjeto','categoria'])
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if len(tracked_dets)>0:
                    ...
                    df_salida=pd.DataFrame(tracked_dets,columns=['x1','y1','x2','y2','idcategoria','pos1', 'pos2','pos3','idobjeto'])
                    df_salida.drop(['x1','y1','x2','y2','pos1', 'pos2','pos3'],axis=1,inplace=True)
                    df_salida=df_salida.astype(int)
                    mapeo=dict(enumerate(names, 0))
                    # creo dataframe para salida de mardown
                    df_salida['categoria']=df_salida.idcategoria.map(mapeo)
                    df= df.append(df_salida,ignore_index=True)
                    contador=df[['idobjeto','categoria']].groupby('categoria')['idobjeto'].nunique()
                    kpi1_text.write(df_salida)
                    kpi2_text.write(contador)


                    for x,y in enumerate(categories):
                        id=str(identities[x])
                        cat=str()
                        text=f"ID: {id} categoria: {cat}"
                            
                        #kpi2_text.write(text)
...
parser.add_argument('--source', type=str, default=ROOT / 'inference/images', help='source')  # file/folder, 0 for webcam
